...
class CppGenerator(api_library_generator.ApiLibraryGenerator):
    ...
    def _DLifyDescription(self, text):
        ...
        dl_partitions = DL_PATTERN_RE.split(
            sentinel + text)  # , 0, re.MULTILINE)
        ...
        for partition in dl_partitions:
            ...
            if len(dl) == 1:
                ...
                # Extra stuff past end of dl that regex cannot detect.
                runoff = ''
                result.append('<dl>\n')
    ...
    def AnnotateApi(self, the_api):
        ...
        monolithic_source_name = the_api.GetTemplateValue(
            'monolithicSourceName')
    ...
    def AnnotateParameter(self, unused_method, parameter):
        ...
        if parameter.data_type.code_type == 'string':
            string_type = self.features.get(
                'stringType') or 'absl::string_view'
    ...
    def AnnotateProperty(self, unused_api, prop, unused_schema):
        ...
        super(CppGenerator, self).AnnotateProperty(
            unused_api, prop, unused_schema)
        prop.SetTemplateValue(
            'isPrimitive', self._IsPrimitiveType(prop.data_type))
        ...
        if parent:
            for ancestor in parent.full_path:
                if ancestor.class_name == safe_class_name:
                    safe_class_name = '%s%s' % (
                        parent.class_name, safe_class_name)
