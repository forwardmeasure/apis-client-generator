...
class ApiTest(absltest.TestCase):
            ...
            if resource.values['wireName'] == 'activities':
                for method in resource.values['methods']:
                    if method.required_parameters:
                        ...
                        self.assertEquals(
                            method.values['parameterOrder'], required_names)
                        tests_executed += 1
    ...
    def testPageable(self):
        ...
        self.assertEquals(list_by_album.values.get(
            'isPagingStyleStandard'), False)
    ...
    def testRepeatedEnum(self):
        ...
        self.assertTrue(isinstance(
            options.data_type, data_types.ArrayDataType))
        self.assertTrue(isinstance(
            options.data_type._base_type, data_types.Enum))
        self.assertEquals(
            'string', options.data_type._base_type.values['type'])
    ...
    def testAuthScope(self):
        ...
        self.assertEquals('A typical scope',
        ...
        self.assertEquals('https://www.googleapis.com/auth/no.description',
                          scope.GetTemplateValue('description'))
        scope = AuthScope(
            api, 'https://www.googleapis.com/auth/trim.slashes//', {})
        ...
        self.assertEquals('mail.google.com',
                          scope.GetTemplateValue('lastPart'))
        self.assertEquals('https://mail.google.com/',
                          scope.GetTemplateValue('value'))
        scope = AuthScope(api,
                          'https://mail.google.com/abc',
                          {'description': 'A non-googleapis.com scope'})
        self.assertEquals('MAIL_GOOGLE_COM_ABC',
                          scope.GetTemplateValue('name'))
        scope = AuthScope(api,
                          'http://mail.google.com/',
                          {'description': 'A non-https scope'})
        self.assertEquals('HTTP___MAIL_GOOGLE_COM',
                          scope.GetTemplateValue('name'))
    ...
    def testPostVariations(self):
        ...
        self.assertTrue(isinstance(m.values.get(
            'responseType'), data_types.Void))
        ...
        for name, schema in api._schemas.items():
            if schema.parent and schema.parent != api:
                ...
                self.assertTrue(name.endswith(
                    '.'.join([parent_wire_name, wire_name])))
    ...
    def testNormalizeUrlComponents(self):
        ...
        # .. in path
        self.assertRaises(ValueError, LoadApi, {
                          'basePath': '/do/not/../go/up'})
    ...
    def testSharedTypes(self):
        ...
        api.VisitAll(lambda o: o.SetLanguageModel(
        ...
        self.assertEquals('com.google.myservice',
                          photos_feed_schema.module.name)
