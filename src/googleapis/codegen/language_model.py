...
class LanguageModel(object):
    ...
    def _Float(self, data_value):
        ...
        value = str(value)
        # Note that str(float()) already appends '.0' if is an integral value.
        return value
    ...
    def ApplyCaseTransform(self, s, policy):
        ...
        if policy.case_transform == LOWER_CASE:
            def transform(s, _): return ''.join(s).lower()
        elif policy.case_transform == UPPER_CASE:
            def transform(s, _): return ''.join(s).upper()
        elif policy.case_transform == UPPER_CAMEL_CASE:
            def transform(s, _): return ''.join([s[0].upper()] + s[1:])
            ...
            def transform(s, _): return utilities.UnCamelCase(''.join(s))
        elif policy.case_transform == UPPER_UNCAMEL_CASE:
            def transform(s, _): return utilities.UnCamelCase(
                ''.join(s)).upper()
            ...
            def transform(s, _): return ''.join(s)
    ...
    def ApplyFormat(self, variable, name, policy):
        ...
        if variable:
            ...
            if hasattr(variable, 'api'):
                ...
                expansions['api_name'] = self.ApplyCaseTransform(
                    api_name, policy)
            if hasattr(variable, 'parent'):
                parent = variable.parent
                if parent:
                    parent_name = parent.GetTemplateValue(
                        'wireName') or parent_name
