...
class Api(template_objects.CodeObject):
    ...
    def _BuildSchemaDefinitions(self):
        ...
        if schemas:
            for name, def_dict in list(schemas.items()):
            ...
            for name, info in list(self._variant_info.items()):
                ...
                for prop in schema.values.get('properties'):
                    ...
                    logging.warn("Variant schema '%s' for base schema '%s' "
                                 ...
                                 info['discriminant'])
                schema.SetTemplateValue(
                    'superClass', info['schema'].class_name)
    ...
    def _NormalizeUrlComponents(self):
        ...
        if not root_url:
            self._api.SetTemplateValue(
                'rootUrl', '%s://%s/' % (scheme, service_host))
...
class Resource(template_objects.CodeObject):
    ...
    def __init__(self, api, name, def_dict, parent=None):
        ...
        super(Resource, self).__init__(
            def_dict, api, parent=parent, wire_name=name)
...
class Method(template_objects.CodeObject):
    ...
    def __init__(self, api, name, def_dict, parent=None):
        ...
        if http_method not in ['DELETE', 'GET', 'OPTIONS', 'PATCH', 'POST', 'PUT',
            ...
            raise ApiException('Unknown HTTP method: %s' %
                               http_method, def_dict)
        if http_method == 'GET':
        ...
        # TODO: return to this
        # req_parameters.sort(key=lambda x, y: cmp(order.index(
        #    x.values['wireName']), order.index(y.values['wireName'])))
    ...
    def _SetUploadTemplateValues(self, upload_protocol, protocol_dict):
        ...
        if upload_path:
            self.SetTemplateValue('%s_upload_path' %
                                  upload_protocol, upload_path)
...
class Parameter(template_objects.CodeObject):
    ...
    def __init__(self, api, name, def_dict, method):
        ...
        if referenced_schema:
            ...
            self._data_type = data_types.PrimitiveDataType(
                def_dict, api, parent=self)
        if self._repeated:
