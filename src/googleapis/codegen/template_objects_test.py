...
class TemplateObjectsTest(absltest.TestCase):
    ...
    def testFullyQualifiedClassName(self):
        ...
        foo._module = template_objects.Module('test',
                                              language_model=self.language_model)
        bar = template_objects.CodeObject(
            {'className': 'Bar'}, None, parent=foo)
        baz = template_objects.CodeObject(
            {'className': 'Baz'}, None, parent=bar)
