...
def main(unused_argv):
    ...
    try:
        ...
        raise app.UsageError(
            'Unsupported language option: %s' % FLAGS.language)
    ...
    if not os.path.isdir(template_dir):
        raise app.UsageError(
            'Can not find template tree at: %s' % template_dir)
...
class TemplateExpander(generator.TemplateGenerator):
    ...
    def GeneratePackage(self, package_writer):
        ...
        self.WalkTemplateTree('.', path_replacements, {},
                              variables, package_writer)
