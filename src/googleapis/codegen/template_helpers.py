    ...
    def RenderText(self, text, context):  # pylint: disable=g-bad-name
        ...
        if self._comment_type == 'doc':
            ...
            continue_prefix = _GetFromContext(context, _DOC_COMMENT_CONTINUE,
                                              _COMMENT_CONTINUE)
            comment_end = _GetFromContext(
                context, _DOC_COMMENT_END, _COMMENT_END)
...
class CommentIfNode(DocCommentNode):
    ...
    def render(self, context):  # pylint: disable=g-bad-name
        """Render the node."""
        try:
            text = django_template.Variable(
                self._variable_name).resolve(context)
            if text:
    ...
    * restores the context.
    ...
    def render(self, context):  # pylint: disable=g-bad-name
        ...
        template_path = os.path.join(
            context['template_dir'], self._template_name)
        # Collect new additions to the context
        newvars = {}
        for target, source in self._bindings.items():
            try:
                newvars[target] = django_template.Variable(
...
def DoCopyrightBlock(parser, unused_token):
    ...
    return DocCommentNode(nodelist=django_template.NodeList([
        django_template.base.VariableNode(
            parser.compile_filter('api.copyright')),
